...
// io.Writer, including the standard library's xlog package.
//
...
// xlog file is then created using original filename.
//
// Whenever a write would cause the current xlog file exceed MaxSize megabytes,
// the current file is closed, renamed, and a new xlog file created with the
// original name. Thus, the filename you give Logger is always the "current" xlog
// file.
//
// Backups use the xlog file name given to Logger, in the form
// `name-timestamp.ext` where name is the filename without the extension,
// timestamp is the time at which the xlog was rotated formatted with the
// time.Time format of `2006-01-02T15-04-05.000` and the extension is the
// original extension.  For example, if your Logger.Filename is
// `/var/xlog/foo/server.xlog`, a backup created at 6:30pm on Nov 11 2016 would
// use the filename `/var/xlog/foo/server-2016-11-04T18-30-00.000.xlog`
//
// Cleaning Up Old Log Files
//
// Whenever a new logfile gets created, old xlog files may be deleted.  The most
...
// Write implements io.Writer.  If a write would cause the xlog file to be larger
// than MaxSize, the file is closed, renamed to include a timestamp of the
// current time, and a new xlog file is created using the original xlog file name.
...
// Rotate causes Logger to close the existing xlog file and immediately create a
// new one.  This is a helper function for applications that want to initiate
// rotations outside of the normal rotation rules, such as in response to
// SIGHUP.  After rotating, this initiates compression and removal of old xlog
...
// openNew opens a new xlog file for writing, moving any old xlog file out of the
...
func (l *Logger) openNew() error {
	...
	if err == nil {
		...
		if err := os.Rename(name, newname); err != nil {
			return fmt.Errorf("can't rename xlog file: %s", err)
		}
...
func (l *Logger) openExistingOrNew(writeLen int) error {
	...
	if err != nil {
		return fmt.Errorf("error getting xlog file info: %s", err)
	}
	...
	if err != nil {
		// if we fail to open the old xlog file for some reason, just ignore
		// it and open a new xlog file.
		return l.openNew()
	}
	l.file = file
	l.size = info.Size()
...
func (l *Logger) filename() string {
	...
	name := filepath.Base(os.Args[0]) + "-rotate.xlog"
	return filepath.Join(os.TempDir(), name)
}

// millRunOnce performs compression and removal of stale xlog files.
// Log files are compressed if enabled via configuration and old xlog
...
func (l *Logger) millRunOnce() error {
	...
	if l.MaxBackups > 0 && l.MaxBackups < len(files) {
		...
		for _, f := range files {
			// Only count the uncompressed xlog file or the
			// compressed xlog file, not both.
			fn := f.Name()
...
func (l *Logger) millRun() {
	for range l.millCh {
		// what am I going to do, xlog this?
		_ = l.millRunOnce()
	}
}

// mill performs post-rotation compression and removal of stale xlog files,
...
// oldLogFiles returns the list of backup xlog files stored in the same
// directory as the current xlog file, sorted by ModTime
func (l *Logger) oldLogFiles() ([]logInfo, error) {
	files, err := ioutil.ReadDir(l.dir())
	if err != nil {
		return nil, fmt.Errorf("can't read xlog file directory: %s", err)
	}
	logFiles := []logInfo{}
...
// max returns the maximum size in bytes of xlog files before rolling.
...
// compressLogFile compresses the given xlog file, removing the
// uncompressed xlog file if successful.
func compressLogFile(src, dst string) (err error) {
	f, err := os.Open(src)
	if err != nil {
		return fmt.Errorf("failed to open xlog file: %v", err)
	}
	defer f.Close()

	fi, err := osStat(src)
	if err != nil {
		return fmt.Errorf("failed to stat xlog file: %v", err)
	}

	if err := chown(dst, fi); err != nil {
		return fmt.Errorf("failed to chown compressed xlog file: %v", err)
	}

	// If this file already exists, we presume it was created by
	// a previous attempt to compress the xlog file.
	gzf, err := os.OpenFile(dst, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, fi.Mode())
	if err != nil {
		return fmt.Errorf("failed to open compressed xlog file: %v", err)
	}
	defer gzf.Close()
	...
	defer func() {
		if err != nil {
			os.Remove(dst)
			err = fmt.Errorf("failed to compress xlog file: %v", err)
		}
	}()
